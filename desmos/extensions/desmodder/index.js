...
const getJszip = () =>
    (jszipPromise ??= import(JSZIP_URL).then((m) => m.default ?? m));

// Cache keys only - nothing is served at this path, but the Cache API insists on a URL.
const desmodderKey = (version, name) =>
    `/desmos/extensions/desmodder/${version}/${name}`;
    ...
    try {
        ...
        await Promise.all(
            Object.entries(files).map(([name, text]) =>
                cache.put(desmodderKey(version, name), new Response(text))
            )
        );
    } catch (_) {
...
async function desmodderLatest() {
    const res = await fetch(
        `${GITHUB_API}/repos/${DESMODDER_REPO}/releases/latest`,
        {
            headers: { accept: "application/vnd.github+json" }
        }
    );
    if (!res.ok)
        throw new Error(`desmodder latest -> ${res.status} ${res.statusText}`);
...
async function desmodderDownload(version) {
    ...
    if (!res.ok)
        throw new Error(
            `desmodder ${version} -> ${res.status} ${res.statusText}`
        );
    ...
    for (const [name, [path]] of Object.entries(DESMODDER_FILES)) {
        ...
        if (!entry)
            throw new Error(
                `desmodder ${version}: ${path} is missing from the release zip`
            );
...
extension({
    ...
    async setup(ctx) {
        ...
        const version =
            requested === "latest" ? await desmodderLatest() : requested;
        if (!TAG.test(version))
            throw new Error(
                `desmodder: ${JSON.stringify(version)} is not a release tag`
            );
        ...
        window.addEventListener("message", (event) => {
            ...
            switch (message.type) {
                case "get-initial-data": {
                    ...
                    const stale =
                        read(KEYS.forceDisabledVersion, "") !== data.version;
                    window.postMessage(
                        {
                            type: "apply-initial-data",
                            pluginsEnabled: read(KEYS.enabled, {}),
                            pluginsForceDisabled: stale
                                ? []
                                : read(KEYS.forceDisabled, []),
