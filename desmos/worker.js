...
function pathAllowed(site, pathname) {
    return (
        !site.allowPath ||
        site.allowPath.some((pattern) => pattern.test(pathname))
    );
}
...
const REWRITABLE =
    /^(?:text\/html|text\/css|text\/javascript|application\/javascript|application\/x-javascript|application\/ecmascript|text\/ecmascript|application\/json|application\/manifest\+json|image\/svg\+xml|text\/plain)/i;
const IS_JS =
    /^(?:text\/javascript|application\/javascript|application\/x-javascript|application\/ecmascript|text\/ecmascript)/i;
...
export default {
    async fetch(request, env, ctx) {
        ...
        if (url.pathname.endsWith(BOOTSTRAP_SUFFIX)) {
            const owner = parsePath(
                url.pathname.slice(0, -BOOTSTRAP_SUFFIX.length)
            );
            if (owner && owner.rest === "/") {
...
function hostPrefix(host) {
    ...
    for (const site of Object.values(SITES)) {
        ...
        if (host.endsWith("." + site.domain)) {
            return (
                site.subPrefix + "/" + host.slice(0, -(site.domain.length + 1))
            );
        }
    }
    return null;
...
function fallback(request, url, env) {
    ...
    if (referer) {
        try {
            ...
            const from =
                ref.origin === url.origin ? parsePath(ref.pathname) : null;
            if (from) {
                return Response.redirect(
                    url.origin + from.prefix + url.pathname + url.search,
                    307
                );
            }
        } catch (_) {}
...
async function proxy(request, url, route) {
    ...
    for (const [name, value] of request.headers) {
        ...
        if (key === "referer") {
            headers.set(
                "referer",
                unproxyUrl(value, url.origin) || target.origin + "/"
            );
            continue;
        }
        if (key === "if-none-match") {
            headers.set("if-none-match", value.replace(/-dp\d+"/g, '"'));
            continue;
        }
    ...
    if (location) {
        out.set(
            "location",
            rewriteText(new URL(location, target).toString(), url.origin)
        );
    }
    ...
    out.set(
        "cache-control",
        capCacheControl(out.get("cache-control"), REWRITTEN_MAX_AGE)
    );
    ...
    if (/^text\/html/i.test(type))
        body = rewriteHtml(body, url.origin, route.prefix);
...
function capCacheControl(value, seconds) {
    ...
    if (/\bmax-age\s*=/i.test(cleaned)) {
        return cleaned.replace(
            /\bmax-age\s*=\s*(\d+)/gi,
            (m, n) => "max-age=" + Math.min(Number(n), seconds)
        );
    }
...
function rewriteCookie(cookie, prefix) {
    return cookie
        ...
        .map((part) =>
            /^path=/i.test(part)
                ? "Path=" + prefix + part.slice(5).replace(/^\/?/, "/")
                : part
        )
        .join("; ");
}
...
const URL_RE =
    /(?<![\w:])(?:(https?|wss?):)?(\\?\/\\?\/)((?:[a-z0-9-]+\.)+[a-z]{2,63})/gi;
...
function rewriteCss(css, prefix) {
    return css
        .replace(
            new RegExp("url\\(\\s*([\"']?)" + ROOT_RELATIVE, "gi"),
            "url($1" + prefix + "/"
        )
        .replace(
            new RegExp("@import\\s+([\"'])" + ROOT_RELATIVE, "gi"),
            "@import $1" + prefix + "/"
        );
}
...
function rewriteHtml(html, origin, prefix) {
    return (
        html
            ...
            .replace(
                new RegExp(
                    "(\\s(?:src|href|action|poster|data-src|formaction)\\s*=\\s*)([\"'])" +
                        ROOT_RELATIVE,
                    "gi"
                ),
                "$1$2" + prefix + "/"
            ...
            .replace(
                ...
                (m, lead, q, list) =>
                    lead +
                    q +
                    list.replace(/(^|,\s*)\/(?!\/|_\/)/g, "$1" + prefix + "/") +
                    q
            )
            // Inline stylesheets need the same url() treatment as external ones.
            .replace(
                /(<style\b[^>]*>)([\s\S]*?)(<\/style>)/gi,
                (m, open, css, close) => open + rewriteCss(css, prefix) + close
            )
            .replace(
                /(\sstyle\s*=\s*)(["'])([^"']*)\2/gi,
                (m, lead, q, css) => lead + q + rewriteCss(css, prefix) + q
            )
...
function clientBootstrap(cfg) {
    ...
    function base() {
        if (!isWorker && typeof document !== "undefined" && document.baseURI)
        ...
        for (var i = 0; i < SITES.length; i++) {
            ...
            if (host.slice(-(site.domain.length + 1)) === "." + site.domain) {
                return (
                    site.subPrefix +
                    "/" +
                    host.slice(0, -(site.domain.length + 1))
                );
            }
        }
        return null;
    ...
    function proxied(u, prefix) {
        ...
        var proto = ws
            ...
            : SECURE
              ...
              : "http:";
        return (
            proto + "//" + AUTHORITY + prefix + u.pathname + u.search + u.hash
        );
    }
    ...
    function rw(input) {
        try {
            ...
            if (
                /^(?:blob:|data:|about:|javascript:|mailto:|tel:|filesystem:)/i.test(
                    u
                )
            )
                return input;
        ...
        g[name] = new Proxy(C, {
            construct: function (Target, args, newTarget) {
                ...
                return Reflect.construct(
                    Target,
                    args,
                    newTarget === g[name] ? Target : newTarget
                );
            }
        });
    }
    ...
    wrapConstructor("Request", function (args) {
        if (
            ...
            (typeof args[0] === "string" ||
                (typeof URL !== "undefined" && args[0] instanceof URL))
        ) {
            args[0] = rw(args[0]);
    ...
    if (isWorker && typeof g.importScripts === "function") {
        ...
        g.importScripts = function () {
            return _importScripts.apply(
                g,
                Array.prototype.map.call(arguments, rw)
            );
        };
    }
    ...
    if (
        ...
        g.navigator.serviceWorker.register = function () {
            return Promise.reject(
                new Error(
                    "service workers are disabled behind the desmos proxy"
                )
            );
        };
    }
    ...
    if (g.Element && g.Element.prototype.setAttribute) {
        ...
        g.Element.prototype.setAttribute = function (name, value) {
            if (name && URL_ATTRS[String(name).toLowerCase()])
                value = rw(value);
