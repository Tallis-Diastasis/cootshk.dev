...
const VF_MODULE_URL =
    "https://cdn.jsdelivr.net/npm/@run-slicer/vf@0.6.3-1.12.0/vf.js";
...
mcVersionFilter.addEventListener("change", () => {
    ...
    if (
        !$("#version-picker").classList.contains("hidden") &&
        currentPickerSlug
    ) {
...
async function searchModrinth(query) {
    ...
    try {
        ...
        const res = await fetch(
            `https://api.modrinth.com/v2/search?query=${encodeURIComponent(query)}&facets=${facets}&limit=15`
        );
        const data = await res.json();
    ...
    `;
        ...
        $$(".search-result", results).forEach((el) => {
            el.addEventListener("click", () =>
                pickProject(el.dataset.slug, el.dataset.id)
            );
        });
    } catch (err) {
...
async function pickProject(slug) {
    ...
    versionPicker.innerHTML =
        '<div class="loading"><div class="spinner"></div><p>Loading versions...</p></div>';

    try {
        const res = await fetch(
            `https://api.modrinth.com/v2/project/${slug}/version?loaders=["fabric"]`
        );
...
function renderVersionPicker() {
    ...
    if (!filtered.length && !allProjectVersions.length) {
        versionPicker.innerHTML =
            '<p class="no-data">No Fabric versions found.</p>';
        return;
    }
  ...
  `;
    ...
    $$(".version-item", versionPicker).forEach((el) => {
        el.addEventListener("click", () =>
            downloadAndAnalyze(el.dataset.url, el.dataset.filename)
        );
    });
}
...
async function parseJar(buffer, filename) {
    ...
    if (!fmjFile) {
        throw new Error(
            `No fabric.mod.json found in ${filename}. This may not be a Fabric mod.`
        );
    }
    ...
    if (firstClass) {
        try {
            ...
            if (
                ...
                javaBytecodeVersion = {
                    major,
                    label: JAVA_VERSIONS[major] || `Java ${major - 44}`
                };
            }
        } catch (_) {
            /* ignore */
    ...
    const textureCount = assetFiles.filter((n) =>
        n.includes("/textures/")
    ).length;
    ...
    const shaderCount = assetFiles.filter((n) =>
        n.includes("/shaders/")
    ).length;

    // 4) Data Pack detection: data/ folder with actual files
    const dataFiles = allFiles.filter(
        (n) => n.startsWith("data/") && !n.endsWith("/")
    );
    const hasDataPack = dataFiles.length > 0;

    const recipeCount = dataFiles.filter((n) => n.includes("/recipes/")).length;
    const lootTableCount = dataFiles.filter((n) =>
        n.includes("/loot_tables/")
    ).length;
    const tagCount = dataFiles.filter((n) => n.includes("/tags/")).length;
    const advancementCount = dataFiles.filter((n) =>
        n.includes("/advancements/")
    ).length;
    const worldgenCount = dataFiles.filter((n) =>
        n.includes("/worldgen/")
    ).length;
    const structureCount = dataFiles.filter(
        (n) => n.includes("/structures/") || n.includes("/structure/")
    ).length;
    ...
    const meta = {
        ...
        dataCounts: {
            ...
            recipes: recipeCount,
            lootTables: lootTableCount,
            tags: tagCount,
            advancements: advancementCount,
            worldgen: worldgenCount,
...
async function getClassBytes(mod, className) {
    ...
    if (!mod.classLookup) {
        mod.classLookup = Object.fromEntries(
            mod.classEntries.map((item) => [item.name, item.entry])
        );
    }
...
async function decompileClassInMod(mod, className) {
    ...
    if (!source) {
        ...
        throw new Error(
            `Decompiler did not return source for class: ${className} in ${modLabel}`
        );
    }
    return source;
}
...
async function lookupModrinthHashes(mod) {
    ...
    try {
        ...
        const projectIds = [
            ...new Set(Object.values(modrinthLookup).map((v) => v.projectId))
        ];
...
function renderMod(mod) {
    ...
    const modrinthUrl = modrinthInfo?.slug
        ? `https://modrinth.com/mod/${modrinthInfo.slug}`
        : null;

    // Breadcrumb for navigation
    const breadcrumb =
        modStack.length > 1
            ? `<div class="breadcrumb">${modStack
                  .map((m, i) => {
                      const name = esc(m.fmj.name || m.fmj.id);
                      if (i === modStack.length - 1)
  ...
  `;
    ...
    if (fmj.breaks && Object.keys(fmj.breaks).length) {
        grid.innerHTML += detailCard(
            "Breaks / Incompatibilities",
            iconSvg("alert"),
            renderBreakList(fmj.breaks),
            "",
            "row-2"
        );
    }
    ...
    if (fmj.entrypoints && Object.keys(fmj.entrypoints).length) {
        grid.innerHTML += detailCard(
            "Entrypoints",
            iconSvg("play"),
            renderEntrypoints(fmj.entrypoints),
            "",
            "row-3"
        );
    }
...
function renderJarItem(mod, index, depth = 0) {
    ...
    const expandable =
        hasDeps || hasEntries || hasMixins || hasJars || fmj.description;
...
function renderCompactMeta(meta) {
    ...
    if (meta.javaBytecodeVersion) {
        tags.push(
            `<span class="tag-compact tag-compact-blue">${esc(meta.javaBytecodeVersion.label)}</span>`
        );
    }
    ...
    if (meta.hasTexturePack) {
        tags.push(
            `<span class="tag-compact tag-compact-green">Textures</span>`
        );
    }
    if (meta.hasDataPack) {
...
function renderJavaMeta(meta) {
    ...
    if (meta.javaBytecodeVersion) {
        pills.push(
            `<div class="meta-pill meta-pill-blue">${iconSvg("cpu")} <strong>${esc(meta.javaBytecodeVersion.label)}</strong> <span class="meta-pill-sub">Bytecode v${meta.javaBytecodeVersion.major}</span></div>`
        );
    }
    ...
    if (meta.classFileCount) {
        pills.push(
            `<div class="meta-pill">${iconSvg("file-code")} ${meta.classFileCount.toLocaleString()} Class Files</div>`
        );
    }

    // Total files
    pills.push(
        `<div class="meta-pill">${iconSvg("archive")} ${meta.totalFileCount.toLocaleString()} Total Files</div>`
    );

    // Mixins
    if (meta.mixinConfigs.length > 0) {
        pills.push(
            `<div class="meta-pill meta-pill-purple">${iconSvg("layers")} ${meta.mixinConfigs.length} Mixin Config${meta.mixinConfigs.length !== 1 ? "s" : ""}</div>`
        );
    } else {
        pills.push(
            `<div class="meta-pill meta-pill-dim">${iconSvg("layers")} No mixins</div>`
        );
    }
    ...
    if (meta.hasTexturePack) {
        ...
        pills.push(
            `<div class="meta-pill meta-pill-green">${iconSvg("image")} Texture Pack <span class="meta-pill-sub">${detail}</span></div>`
        );
    } else {
        pills.push(
            `<div class="meta-pill meta-pill-dim">${iconSvg("image")} No texture pack</div>`
        );
    }
    ...
    if (meta.hasDataPack) {
        ...
        pills.push(
            `<div class="meta-pill meta-pill-orange">${iconSvg("database")} Data Pack <span class="meta-pill-sub">${detail}</span></div>`
        );
    } else {
        pills.push(
            `<div class="meta-pill meta-pill-dim">${iconSvg("database")} No data pack</div>`
        );
    }
    ...
    let mixinDetail = "";
    if (meta.mixinConfigs.length) {
        mixinDetail = `<div class="meta-detail"><h4>Mixin config files</h4><div class="mixin-list">${meta.mixinConfigs
            .map(
                (m) => `
...
function renderDecompilerPanel(mod) {
    ...
    const activeMinecraftVersion =
        detectedMinecraftVersion || fallbackMinecraftVersion;
    const versionOptions = mcVersions
        .map(
            (v) =>
    ...
    const mappingOptions = MC_NAMESPACES.map(
        (ns) =>
...
function resolveClassEntry(mod, classRef, sourceContext = null) {
    ...
    if (mod.classByDotted.has(normalized))
        return mod.classByDotted.get(normalized);
    ...
    if (!normalized.includes(".") && sourceContext) {
        ...
        if (imported && mod.classByDotted.has(imported))
            return mod.classByDotted.get(imported);
        if (sourceContext.packageName) {
            const inPackage = `${sourceContext.packageName}.${normalized}`;
            if (mod.classByDotted.has(inPackage))
                return mod.classByDotted.get(inPackage);
        }
    }
...
function analyzeDeclarations(source) {
    ...
    const methodPattern =
        /^\s*(?:@\w+(?:\([^)]*\))?\s*)*(?:public|protected|private|static|final|abstract|synchronized|native|strictfp|default|\s)+[\w<>\[\],?.@ ]+\s+([A-Za-z_$][\w$]*)\s*\([^;{)]*\)\s*(?:throws[^{]+)?\{/;
    const classPattern =
...
function addMinecraftTreeEntry(panel, canonicalInterm, displayName) {
    ...
    const existing = $(
        `.minecraft-tree-link[data-canonical="${CSS.escape(canonicalInterm)}"]`,
        list
    );
    if (existing) {
        existing.textContent = displayName;
        $$(".minecraft-tree-link.active", list).forEach((el) =>
...
function renderHighlightedSource(source, mod, className, panel) {
    ...
    const renderedLines = lines.map((line, lineIndex) => {
        ...
        while (i < line.length) {
            ...
            if (/\d/.test(ch)) {
                let j = i + 1;
                while (j < line.length && /[\d_\.xXa-fA-FLlFfDd]/.test(line[j]))
                    j += 1;
            ...
            if (/[A-Za-z_$]/.test(ch)) {
                ...
                if (classDecl && token === classDecl)
                    declarationToken = { type: "class" };
                if (methodDecl && token === methodDecl)
                    declarationToken = { type: "method" };
                out += renderIdentifier(
                    token,
                    lineIndex,
                    nextChar,
                    declarationToken
...
async function openClassInPanel(panel, mod, className, status) {
    ...
    const classBtn = $(
        `.class-tree-file[data-class-name="${CSS.escape(classEntry.name)}"]`,
        panel
    );
    if (classBtn) {
        $$(".class-tree-file.active", panel).forEach((el) =>
            el.classList.remove("active")
        );
    ...
    if (cached) {
        code.innerHTML = renderHighlightedSource(
            await applyModMappings(panel, cached),
            mod,
            classEntry.name,
            panel
        );
    ...
    try {
        ...
        code.innerHTML = renderHighlightedSource(
            await applyModMappings(panel, source),
            mod,
            classEntry.name,
            panel
        );
...
async function exportAllSources(mod, panel, status, btn) {
    ...
    const entries = (mod.classEntries || []).filter(
        (e) => !e.name.split("/").pop().includes("$")
    );
    if (!entries.length) {
        status.textContent = "No classes to export.";
        return;
    }
    ...
    try {
        ...
        if (!mod.classResources)
            mod.classResources = mod.classEntries.map((i) => i.name);
        ...
        for (let i = 0; i < entries.length; i += BATCH) {
            ...
            for (const e of batch) {
                ...
                if (mc) {
                    try {
                        src = await mc.remapModSource(version, namespace, src);
                    } catch (_) {
                        /* keep raw */
                    }
                }
        ...
        const blob = await zip.generateAsync(
            ...
            (meta) => {
                status.textContent = `Packaging ${done} files… ${meta.percent.toFixed(0)}%`;
            }
        );
        const nsTag = remap ? `-${namespace}` : "";
        const base = String(mod.fmj?.id || mod.filename || "sources").replace(
            /[^\w.-]+/g,
            "_"
        );
...
function setupDecompilerButtons() {
    $$(".decompiler-panel").forEach((panel) => {
        ...
        tree.addEventListener("click", async (e) => {
            ...
            $$(".class-tree-file.active", tree).forEach((el) =>
                el.classList.remove("active")
            );
            classBtn.classList.add("active");
        ...
        code.addEventListener("click", async (e) => {
            ...
            if (jumpType === "method") {
                ...
                const methodTarget = $(
                    `#method-target-${CSS.escape(method)}`,
                    code
                );
                methodTarget?.scrollIntoView({
                    behavior: "smooth",
                    block: "center"
                });
                return;
...
function mcStoredNamespace() {
    try {
        return localStorage.getItem(MC_NS_STORAGE_KEY);
    } catch (_) {
        return null;
    }
}
...
function mcStoreNamespace(ns) {
    try {
        localStorage.setItem(MC_NS_STORAGE_KEY, ns);
    } catch (_) {
        /* ignore */
    }
}
...
async function loadMcClass(panel, className, status) {
    const version = panel.dataset.mcVersion;
    if (!version) {
        status.textContent = "No Minecraft version selected.";
        return;
    }

    const environment = (panel.dataset.mcEnv || "client,server")
        .split(",")
        .filter(Boolean);
    ...
    try {
        ...
        const {
            ...
            canonicalInterm,
            namespace: usedNs
        } = await mc.decompileMinecraftClass({
            version,
            namespace,
            environment,
            name: className
...
function renderDepList(deps) {
    return `<div class="dep-list">${Object.entries(deps)
        .map(
            ([id, ver]) => `
    <div class="dep-item">
...
function renderBreakList(breaks) {
    return `<div class="break-list">${Object.entries(breaks)
        .map(
            ([id, ver]) => `
...
function renderEntrypoints(entrypoints) {
    return `<div class="entry-list">${Object.entries(entrypoints)
...
function renderMixins(mixins) {
    return `<div class="mixin-list">${mixins
        .map(
            (m) => `
...
function renderContact(contact) {
    return `<div class="contact-links">${Object.entries(contact)
        .map(
            ([key, url]) => `
...
function iconSvg(name) {
    const icons = {
        ...
        "file-code":
            '<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="10" y1="12" x2="8" y2="15"/><line x1="10" y1="18" x2="8" y2="15"/><line x1="14" y1="12" x2="16" y2="15"/><line x1="14" y1="18" x2="16" y2="15"/></svg>',
        archive:
