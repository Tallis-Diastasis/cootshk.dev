...
let VF_MODULE_URL =
    "https://cdn.jsdelivr.net/npm/@run-slicer/vf@0.6.3-1.12.0/vf.js?tailor-mc";
const ASM_MODULE_URL =
    "https://cdn.jsdelivr.net/npm/@run-slicer/asm@0.17.0/+esm";
const ZIP_MODULE_URL =
    "https://cdn.jsdelivr.net/npm/@run-slicer/zip@0.5.2/+esm";
...
export function _setVfUrl(url) {
    VF_MODULE_URL = url;
}

export const MC_VERSION_MANIFEST_URL =
...
let vfPromise = null,
    ...
    zipPromise = null;
const getVf = () =>
    (vfPromise ??= import(VF_MODULE_URL).then((m) => m.decompile));
...
async function resolveYarnBuild(version) {
    ...
    const re = new RegExp(
        `<version>(${version.replace(/[.]/g, "\\.")}\\+build\\.(\\d+))</version>`,
        "g"
    );
    let best = null,
        bestNum = -1,
...
function makeClassMapper(classObf2Target) {
    const mapPlain = (name) => {
        ...
        if (dollar > 0)
            return (
                mapPlain(name.slice(0, dollar)) + "$" + name.slice(dollar + 1)
            );
        return name;
    };
    return mapPlain;
...
const mapDescriptor = (desc, mapPlain) =>
    desc.replace(/L([^;]+);/g, (_, n) => "L" + mapPlain(n) + ";");
const remapSignature = (sig, mapPlain) =>
    sig.replace(/L([A-Za-z0-9_$/]+)/g, (_, n) => "L" + mapPlain(n));
const mapClassEntryName = (name, mapPlain) =>
    name.charCodeAt(0) === 91 /* '[' */
        ? mapDescriptor(name, mapPlain)
        : mapPlain(name);
            ...
            if (m) {
                ...
                const args = m[3].trim()
                    ? m[3]
                          .split(",")
                          .map((a) =>
                              proguardSourceTypeToJvm(a.trim(), official2obf)
                          )
                          .join("")
                    : "";
                methodMap.set(
                    `${cls.obfOwner}\0${m[4]}\0(${args})${ret}`,
                    m[2]
                );
                continue;
            }
...
export function parseTinyV2(text) {
    ...
    for (let li = 1; li < lines.length; li++) {
        ...
        if (p[0] === "c") {
            ...
            if (p[1] === "m")
                methods.push({ owner: curObf, desc, obf, interm, named });
...
async function buildMappingTable(version, namespace) {
    ...
    const promise = (async () => {
        ...
        if (yarnBuild) {
            const zip = await getZip();
            const jarBlob = await cachedFetchBlob(
                `${YARN_MAVEN_BASE}/${yarnBuild}/yarn-${yarnBuild}-mergedv2.jar`
            );
            const archive = await zip.readBlob(jarBlob);
            const entry = archive.entries.find(
                (e) => e.name === "mappings/mappings.tiny"
            );
        ...
        if (tiny) {
            for (const r of tiny.records) {
                if (r.interm) {
                    obf2interm.set(r.obf, r.interm);
                    interm2obf.set(r.interm, r.obf);
                }
            }
        }
        ...
        if (namespace === "mojang") {
            const meta = await getVersionMeta(version);
            const url =
                meta.downloads?.client_mappings?.url ||
                meta.downloads?.server_mappings?.url;
            if (!url)
                throw new Error(`No Mojang mappings published for ${version}`);
            ...
            for (const [interm, obf] of interm2obf)
                addClassSrc(interm, classObf2Target.get(obf));
            if (tiny) {
                for (const m of tiny.methods) {
                    const off = pg.methodMap.get(
                        `${m.owner}\0${m.obf}\0${m.desc}`
                    );
            ...
            const pick = (interm, named) =>
                namespace === "loom" ? named || interm : interm;
            for (const r of tiny.records) {
            ...
            for (const m of tiny.methods) {
                ...
                if (target)
                    methodMap.set(`${m.owner}\0${m.obf}\0${m.desc}`, target);
                if (m.interm && m.named && m.named !== m.interm)
                    memberToTarget.set(m.interm, m.named);
            }
            for (const f of tiny.fields) {
                const target = pick(f.interm, f.named);
                if (target) fieldMap.set(`${f.owner}\0${f.obf}`, target);
                if (f.interm && f.named && f.named !== f.interm)
                    memberToTarget.set(f.interm, f.named);
            }
        } else {
        ...
        return {
            ...
            toObf(name) {
                return (
                    reverseTargetClass.get(name) || interm2obf.get(name) || null
                );
            },
            toInterm(name) {
...
const ASM_UTF8 = 1,
    ...
    ASM_FIELDREF = 9,
    ASM_METHODREF = 10,
    ASM_IFACE_METHODREF = 11,
    ASM_NAME_AND_TYPE = 12,
    ASM_METHOD_TYPE = 16,
    ASM_DYNAMIC = 17,
    ASM_INVOKE_DYNAMIC = 18;
...
async function createRemapper(table, jar) {
    ...
    async function hierarchyOf(obfName) {
        ...
        try {
            ...
            if (bytes) {
                ...
                if (node.superClass)
                    info.superName = node.pool[node.superClass.name].string;
                info.interfaces = node.interfaces.map(
                    (i) => node.pool[i.name].string
                );
            }
        } catch (_) {
            /* keep empty */
    ...
    async function remapClass(bytes) {
        ...
        for (const e of entries) {
            if (e && e.type === ASM_UTF8 && !utf8Map.has(e.string))
                utf8Map.set(e.string, e);
        }
        const intern = (str) => {
        ...
        for (const e of entries) {
            ...
            if (
                e.type === ASM_FIELDREF ||
                e.type === ASM_METHODREF ||
                e.type === ASM_IFACE_METHODREF
            ) {
                refs.push(e);
            } else if (
                e.type === ASM_DYNAMIC ||
                e.type === ASM_INVOKE_DYNAMIC
            ) {
                const nt = pool[e.nameType];
                e.nameType = internNameType(
                    pool[nt.name].string,
                    table.mapDesc(pool[nt.type_].string)
                ).index;
            } else if (e.type === ASM_METHOD_TYPE) {
                e.descriptor = intern(
                    table.mapDesc(pool[e.descriptor].string)
                ).index;
            }
        }
        for (const e of refs) {
        ...
        for (const e of entries) {
            if (e && e.type === ASM_CLASS) {
                e.name = intern(
                    mapClassEntryName(classObf.get(e), table.mapClass)
                ).index;
            }
        }
        ...
        const remapAttrs = (attrs) => {
            for (const a of attrs) {
                if (a.type === "Signature" && a.signatureEntry) {
                    a.signatureEntry = intern(
                        table.mapSig(a.signatureEntry.string)
                    );
                    a.dirty = true;
                    ...
                    for (const ic of a.classes) {
                        if (ic.innerNameEntry && ic.innerEntry) {
                            ...
                            ic.innerNameEntry = intern(
                                full.split("/").pop().split("$").pop()
                            );
                        }
                    }
                    a.dirty = true;
...
function isDecompileFailure(src) {
    return (
        !src ||
        src.includes("$VF: Couldn't be decompiled") ||
        src.includes("// $VF:")
    );
}
...
export async function remapModSource(version, namespace, source) {
    ...
    let table;
    try {
        table = await buildMappingTable(version, namespace);
    } catch (_) {
        return source;
    }
    ...
    let out = source.replace(
        /net\.minecraft\.(class_\d+(?:\$class_\d+)*)/g,
        (m, cls) => {
    ...
    out = out.replace(
        /\b(class_\d+|method_\d+|field_\d+)\b/g,
        (tok) =>
            (tok.charCodeAt(0) === 99
                ? classSimpleToTarget.get(tok)
                : memberToTarget.get(tok)) || tok
    );
    return out;
}
...
export async function decompileMinecraftClass({
    version,
    namespace,
    environment,
    name
    ...
    if (namespace) {
        ...
        try {
            ...
            table = null;
        } // no mappings published for this version
        if (table) {
            ...
            if (obf) {
                ...
                const sourceFn = async (n) => {
                    ...
                    if (!b) return null;
                    try {
                        return await remapper.remapClass(b);
                    } catch (_) {
                        return null;
                    }
                };
                const result = await decompile([targetName], {
                    resources: table._resources,
                    source: sourceFn,
                    options: { banner: DECOMPILE_BANNER }
                });
                const source = result[targetName];
                if (!source)
                    throw new Error(
                        `Decompiler returned no source for ${targetName.replace(/\//g, ".")}`
                    );
                if (!isDecompileFailure(source))
                    writeSourceCache(cacheKey, source);
    ...
    if (!jar.has(slash))
        throw new Error(`Class not found in Minecraft ${version}: ${name}`);
    ...
    if (cached != null)
        return {
            source: cached,
            targetName: slash,
            canonicalInterm: slash,
            namespace: null
        };
    const result = await decompile([slash], {
        resources: [...jar.classNames()],
        source: (n) => jar.bytes(n),
        options: { banner: DECOMPILE_BANNER }
    });
